...
node {
    ...
    ws("${gopath}/src/github.com/ONSdigital/dp-dd-csv-transformer") {
        stage('Checkout') {
            checkout scm
            sh 'git clean -dfx'
        ...
        stage('Build') {
            sh "GOPATH=${gopath} go build -o build/dp-dd-csv-transformer"
        }
        ...
        stage('Image') {
            docker.withRegistry("https://${env.ECR_REPOSITORY_URI}", { ->
                docker.build('dp-dd-csv-transformer', '--no-cache --pull --rm .').push(revision)
            })
        }
        ...
        stage('Bundle') {
            ...
            sh "tar -cvzf dp-dd-csv-transformer-${revision}.tar.gz appspec.yml scripts/codedeploy"
            sh "aws s3 cp dp-dd-csv-transformer-${revision}.tar.gz s3://${env.S3_REVISIONS_BUCKET}/"
        }
